...
\textcolor{blue}{Table \ref{tab:zerooutliers} shows the specificity for experiments 1-3 when no outliers are present. A lower specificity indicates the presence of false positives. We see that the average specificity of lookout is greater than $0.99$ with low standard deviation values. Stray has an average specificity of 1 with zero standard deviation and HDoutliers has similar specificity for all three experiments. We see that lookout has comparable specificity to HDoutliers and stray, demonstrating that the false positive rate is low. }
