...
% =======================================================================
% Outlier detection - applications and importance
% Different techniques  - need for robust methods
% Extreme value theory methods for outlier detection
% These methods based on distances
% Kernel density estimate methods - bandwidth
% Our contributions -  bridging topological data analysis to KDE
% And outlier persistence 
% R package - lookout
% What the different sections represent
...
%A brief introduction to persistent homology and the bandwidth selection process is given in Section~\ref{sec:methodology}.  Using leave one out kernel density estimates and the block maxima approach in EVT we identify outliers. We give a short overview of EVT and the block maxima approach in  Section~\ref{sec:methodology}. 
%A brief introduction to persistent homology and EVT is given in Section~\ref{sec:methodology}. 
...
%the effect of bandwidth and the level of significance on outlier detection. 
...
%Bandwidth selection for outlier detection is different from selecting  in kde bandwidth for outlier detection is different 

%Both stray and HDoutliers use the Spacings Theorem, which is detailed in \cite{Burridge2006}.  
%While the term outlier detection has been around for several decades and was predominantly used by statisticians in the past, the terms anomaly and novelty detection are not as old and is used by computer scientists and statisticians alike. 

% In fact, both HDoutliers \citep{wilkinson2017visualizing} and stray \citep{pridiltal} use EVT in their outlier detection framework. Both HDoutliers and stray use distances to detect outliers. We investigate a EVT based framework using 
...
% Given a point cloud of data, the resulting Rips complex depends on the value of $\epsilon$. As such, the question arises which $\epsilon$ is most representative of the structure of the data cloud. For example the point cloud illustrated in Figure~\ref{fig:annulus} lies in an annulus. The figure on the top left shows the Rips complex using $\epsilon = 0.1$. This Rips complex consists of mostly disconnected individual points and a small number of edges. The figure at the top right shows the Rips complex using $\epsilon = 0.3$. This consists of $3$ connected components having edges, triangles, tetrahedrons and possibly other $k$-simplices. The figure at the bottom left shows the Rips complex for $\epsilon = 0.8$, which shows 1 connected component consisting of different $k$-simplices and 1 hole. This Rips complex is desirable from a data analysis point of view as it identifies the annulus well. The figure at the bottom right shows the Rips complex for $\epsilon = 1.5$, which connects everything together and closes up the hole. 
...
\end{equation}
where $x_j$ is left out. We further simplify it and obtain %This gives the leave-one-out estimate at $x_j$ by leaving $x_j$ out as
...
%For the Gaussian kernel we use $h = d_*$ and for Epanechnikov kernel we use $h = \sqrt{5} d_*$ so that 
...
% TALK ABOUT EXISTING LITERATURE FOR OUTLIER DETECTION USING KDE BEFORE
...
% Next we consider the choice of Rips diameter. We are interested in capturing the structure of the point cloud. As such, we choose a diameter that gives rise to persisting features, which are connected components for dimension $0$. We consider the Rips diameter intervals $(d_i, d_{i+1})$ for all $i$, and find the largest interval by computing successive differences 
% \begin{equation}\label{eq:deathrad1}
%     \Delta d_i = d_{i+1} - d_i\, ,  \quad \text{for} \quad i \in \{1, \cdots,  N-1 \} \, . 
% \end{equation}
...
% As we want a somewhat bigger bandwidth we consider the top portion of this sequence, that is a subsequence containing the top $k\%$ of the death diameters, where $k$ is set to a default value of $50$.  We compute the 

   

% We use the death diameter of the connected components to select a bandwidth for our kernel density estimation. 
...
%The algorithm \textit{lookout} is based on leave-one-out kernel density estimates. 
...
%In addition, the point with index 495 is also identified as an outlier up until a bandwidth of 1.12. 
...
%10 discrete significance levels $\alpha \in \{0.01, \, 0.02, \, \ldots, \, 0.1 \}$.   \in  (0, \, 0.1)$
...
\end{equation}
Thus, if a point is identified as an outlier with a significance level  $ \alpha = 0.01$, then it has  strength  10, and  an outlier with $\alpha = 0.1$ has strength 1. To compute persistence over significance levels, the only modification that needs to be done to Algorithm~\ref{algo:persistence} is to fix $\alpha = 0.1$ and to record the minimum significance level   %$P\left(-\log(y_{-i})|\mu_0, \sigma_0, \xi_0 \right)$ 
...
% \subsubsection{Outlier persistence as a tool for tuning parameters}
% Often, in real applications outliers are context dependent, i.e. a fixed definition of an outliers will not be effective as each application has their own definition of an outlier \citep{leigh2019framework,normalizationoutliers}. We can use outlier persistence to cater for such a context. For example, consider the ******


%Even though the bandwidth is depicted as a continuous parameter in these graphs, when computing outlier persistence we use a discrete set of bandwidth values. 
% identify outliers using the algorithm lookout at discrete bandwidth values starting from a small bandwidth and ending in a large bandwidth. 

% =======================================================================
% =======================================================================
\section{Experiments with synthetic data}\label{sec:simulations}
% =======================================================================
In this Section we first explore outlier persistence using simple examples. % and compare it with outliers identified from lookout using $\alpha = 0.1$. %We use $\alpha = 0.1$ so that we can match lookout outliers with the outlier persistence diagram. 
...
%The persistence diagrams show that the high strength outliers persist for a bigger range of bandwidth values and also that lookout finds a suitable bandwidth for detecting these outliers.  
...
% In fact, the medians of lookout - HDoutliers and lookout - stray performance values are  0.131 and 0.138 respectively. A Student's t-test on the differences between lookout and HDoutliers gives a mean of 0.048 with a 95\% confidence interval $(0.038, \, 0.059)$.  For differences between lookout and stray we get  a mean of 0.084 with a 95\% confidence interval $(0.074\, , 0.093)$ from the t-test. The bottom graph shows the letter-value plots of the F-measure results and again we see that the area above zero is bigger than the area below and the median, denoted by a white line is above zero. The median is 0.0405 and 0.0406 for lookout - HDoutliers and lookout - stray respectively. The Student's t-test for lookout - HDoutliers gives a mean of 0.0711 with a 95\% confidence interval of $(0.066,\,  0.076)$ and
...
% We see that the letter value plots have a larger area when Gmean and Fmeasure  are positive, compared to the negative portion. This signifies that more datasets have a positive Gmean and Fmeasure for lookout - HDoutliers and lookout - stray performance values compared to the negative values. 
% =======================================================================
% =======================================================================
...
The outlier detection data repository used in Section~\ref{sec:applications} is available at \cite{datasets}. %\url{https://bridges.monash.edu/articles/dataset/Datasets_12338_zip/7705127/4}.  
